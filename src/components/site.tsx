...
const ROBLOX_GAME_URL = "https://www.roblox.com/games/16146832113/Anime-Vanguards-Eternal-Adversaries";
const OFFICIAL_SITE_URL = "https://vanguards.gg/";
const DISCORD_URL = "https://discord.com/invite/animevanguards";
const YOUTUBE_CHANNEL_URL = "https://www.youtube.com/@animevanguardsofficial";
  ...
  const header = (
    <div className="flex items-center justify-between gap-4">
      <Link href={localizeHref("/", locale)} className="flex items-center gap-3">
        <span className="grid h-9 w-9 place-items-center overflow-hidden rounded-xl border border-border bg-muted p-1">
          <img src="/android-chrome-192x192.png" alt="Anime Vanguards Wiki icon" className="size-full object-contain" />
        </span>
        <span className="text-sm font-bold tracking-wide text-foreground">Anime Vanguards</span>
      </Link>
      ...
      <section className="rounded-2xl border border-border bg-card/60 p-5">
        ...
        <div className="space-y-3 text-sm">
          <div className="rounded-xl bg-muted p-3">
            <code className="font-bold text-foreground">DMCAFree</code>
            <p className="mt-1 text-muted-foreground">100 Trait Rerolls</p>
          </div>
          <div className="rounded-xl bg-muted p-3">
            <code className="font-bold text-foreground">No second code yet</code>
            <p className="mt-1 text-muted-foreground">Waiting for another officially indexed Anime Vanguards code.</p>
          </div>
  ...
  return (
    <div className="group relative cursor-pointer overflow-hidden rounded-2xl border border-border shadow-lg transition-all duration-200">
      <div className="relative aspect-video w-full">
        <img src="/images/hero-trailer-thumbnail.jpg" alt="Anime Vanguards Eternal Adversaries showcase" className="size-full object-cover transition-all duration-200 group-hover:brightness-80" />
      </div>
...
export function TrailerDialog({ videoId }: { videoId: string }) {
  ...
  return (
    <dialog id="trailer-dialog" className="fixed inset-0 z-[100] m-0 flex h-screen max-h-none w-screen max-w-none items-center justify-center border-0 bg-black/80 p-0 backdrop-blur-sm opacity-0 pointer-events-none transition-opacity duration-200" onCancel={(e) => { e.preventDefault(); closeTrailer(); }} onClick={(e) => { if (e.target === e.currentTarget) closeTrailer(); }}>
      <div className="relative w-full max-w-4xl mx-4">
        <iframe id="trailer-iframe" title="Anime Vanguards video player" className="aspect-video w-full rounded-xl" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerPolicy="strict-origin-when-cross-origin" allowFullScreen />
        <button type="button" className="absolute right-3 top-3 z-10 rounded-md bg-black/70 px-2.5 py-1 text-sm font-medium text-white/90 backdrop-blur-sm hover:text-white" onClick={closeTrailer}>Close</button>
      </div>
    </dialog>
  );
}
...
export function TrailerButton({ videoId }: { videoId: string }) {
  return (
    <>
      <button type="button" className="w-full" aria-haspopup="dialog" onClick={() => { const d = document.getElementById("trailer-dialog") as HTMLDialogElement; const f = document.getElementById("trailer-iframe") as HTMLIFrameElement; f.src = `https://www.youtube.com/embed/${videoId}?autoplay=1&rel=0`; d.showModal(); d.classList.remove("opacity-0", "pointer-events-none"); d.classList.add("opacity-100", "pointer-events-auto"); }}>
